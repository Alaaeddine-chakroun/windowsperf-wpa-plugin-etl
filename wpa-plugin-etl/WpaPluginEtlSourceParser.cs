...
{
    public class WpaPluginEtlSourceParser
        : ISourceParser<ReadGPCEvent, WpaPluginEtlSourceParser, string>
    {
        ...
        public void ProcessSource(
            ISourceDataProcessor<ReadGPCEvent, WpaPluginEtlSourceParser, string> dataProcessor,
            ILogger logger,
        ...
        )
        {
                    ...
                    parser.All += delegate(TraceEvent data)
                    {
                        ...
                        if (data.ProviderName.IndexOf("WindowsPerf Driver") != -1)
                        {
                            ReadGPCEvent ev = new ReadGPCEvent
                            {
                                ...
                                Time = Timestamp.FromNanoseconds(
                                    stamp.ToNanoseconds - startTime.ToNanoseconds
                                ),
            ...
            DataSourceInfo = new DataSourceInfo(
                0,
                (endTime - startTime).ToNanoseconds,
                firstEvent.ToUniversalTime()
            );
